...
학습 데이터 800건
        │
        │  ※ LayoutLMv3는 텍스트만 보는 게 아니라
        │    "이 단어가 문서 어디에 있는지(위치)"와
        │    "주변이 어떻게 생겼는지(이미지)"를 함께 봄
        │
        ▼ 모델에 한 세트씩 입력
[텍스트 + bbox 위치 + 이미지] → LayoutLMv3
        │
        ▼ 모델이 각 단어에 BIO 태그를 예측
예측:  "9.76" → B-UNIT_PRICE
정답:  "9.76" → B-UNIT_PRICE  ← 맞으면 손실 낮음
        │
        │  ※ 처음에는 모델이 틀리게 예측함
        │    틀린 만큼을 수치로 계산해서 (손실)
