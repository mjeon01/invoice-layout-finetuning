  ...
  **LayoutLMv3 입력 구성:**
  ...
  LayoutLMv3는 글자 자체뿐만 아니라 **글자의 위치(Layout)** 를 중요하게 학습하는 모델입니다.

  - **정렬의 중요성**: XY Cut으로 읽는 순서를 사람처럼 맞춰주면 모델이 문서 구조를 훨씬 잘 이해
  - **시너지 효과**: 정렬된 입력(XY Cut) + 명확한 이름표(BIO)가 결합되면, 복잡한 수산물 인보이스에서도 필요한 정보를 정확히 추출 가능
  ...
  ─────────────────────────────────────────────────────────────────
    ...
    합성 인보이스 PDF 1,000건
    (실제 인보이스와 유사한 구조이지만, 업체명·금액·품목은 모두 가상)
            │
            │  ※ 합성 PDF는 컴퓨터가 직접 만들었기 때문에
            │    각 단어가 페이지 어디에 있는지 정확히 알고 있습니다.
            │    → 따로 사람이 위치를 표시할 필요 없음
            │
            ▼ 각 단어의 위치(좌표) 자동 추출
    단어 목록 + 페이지 내 위치 좌표 (오차 없음)
            │
            │  ※ 모델이 학습하려면 각 단어가 어떤 필드인지
            │    레이블가 있어야 합니다.
            │    정답 파일에서 값을 찾아 자동으로 붙여줍니다.
            │
            ▼ 단어마다 레이블 자동 부여
      "PT."    → 수출업체 시작
      "INDO"   → 수출업체 계속
